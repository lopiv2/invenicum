...
class _ScraperGridScreenState extends State<ScraperGridScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final scrapers = provider.scrapersForContainer(
                            ...
                            IconButton(
                              ...
                              onPressed: () async {
                                final confirm = await showAppDialog<bool>(
                                  ...
                                  title: l10n.confirmDeleteScraper,

                                  body: Text(
                                    l10n.confirmDeleteScraperMessage(s.name),
                                  ),
                                ...
                                if (confirm == true) {
                                  try {
                                    await provider.deleteScraper(
                                      int.parse(widget.containerId),
                                      s.id,
                                    );
                                    ToastService.success(
                                      l10n.scraperDeletedSuccess,
                                    );
                                  } catch (e) {
                                    ToastService.error(
                                      l10n.errorDeletingScraper(e.toString()),
                                    );
                                  }
                                }
                              },
