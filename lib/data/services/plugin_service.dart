...
import 'package:invenicum/core/utils/sdk_plugin_parser.dart';
...
class PluginService {
  ...
  Future<Map<String, dynamic>> updatePlugin(
    String id, // Pasamos el ID por separado para construir la URL
    Map<String, dynamic> pluginData,
  ) async {
    try {
      // 💡 Enviamos el PUT a /plugins/:id
      // El backend recibirá el token en el Header (vía tu Interceptor de Dio)
      // y determinará si hace Commit directo o crea un Pull Request.
      final response = await _dio.put('/plugins/$id', data: pluginData);

      return Map<String, dynamic>.from(response.data);
    } on DioException catch (e) {
      // Manejo de errores específico para feedback en UI
      throw Exception(
        e.response?.data['message'] ?? "Error al actualizar el plugin",
      );
    }
  }
